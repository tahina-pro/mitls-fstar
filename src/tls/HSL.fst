...
let st_forall_intro (#a:Type0) (#b:(a -> Type0))
		    (#pre:(a -> st_pre))
		    (#post:(x:a -> TLSMem.mem -> st_post (b x)))
...
let st_forall_intro_2 (#a:Type0) (#b:(a -> Type0)) (#c:(x:a -> b x -> Type0))
		     ...
		     (#post:(x:a -> y:b x -> TLSMem.mem -> st_post (c x y)))
...
//TLSMem should prove that mem is inhabited to eliminate one of the quantifiers below
assume val some_mem : TLSMem.mem
