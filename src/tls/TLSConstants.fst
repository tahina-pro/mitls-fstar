...
include Parsers.Parse_cipherSuite // for cipherSuite constructors
...
type cipherSuite' =
  ...
  | SCSV           : cipherSuite'
  | UnknownCipherSuite: (b: cipherSuite_repr { not (known_cipherSuite_repr b) }) -> cipherSuite'
...
inline_for_extraction
type cipherSuiteName = cipherSuite
...
val name_of_cipherSuite : cipherSuite' -> Tot (result cipherSuiteName)
...
let name_of_cipherSuite_of_name : (n: cipherSuite) -> Tot (squash (match cipherSuite_of_name n with Some c -> name_of_cipherSuite c == Correct n | _ -> True)) =
  _ by (
...
let cipherSuite_of_name_of_cipherSuite (c: cipherSuite') : Tot (squash (match name_of_cipherSuite c 
with Correct n -> cipherSuite_of_name n == Some c | _ -> True)) = ()
...
let valid_cipher_suites = cs: list valid_cipher_suite
...
// Only to be invoked with TLS 1.2 (hardcoded in previous versions)
val prfMacAlg_of_ciphersuite_aux: valid_cipher_suite -> Tot (option macAlg)
...
let pvcs (pv:protocolVersion) (cs:valid_cipher_suite) =
  match pv, cs with
...
val sessionHashAlg: pv:protocolVersion -> cs:valid_cipher_suite{pvcs pv cs} -> Tot hashAlg
...
val get_aeAlg: cs:valid_cipher_suite{ CipherSuite? cs \/ CipherSuite13? cs } -> Tot aeAlg
let get_aeAlg cs =
  match cs with
...
val defaultCertTypes: bool -> valid_cipher_suite -> HyperStack.All.ML (l:list certType{List.Tot.length l <= 1})
