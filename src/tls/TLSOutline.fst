...
// assume val partner: epoch -> TLSMem.t -> Tot epoch
...
assume val c_regionsT:connection -> hh -> Tot (Set.set TLSMem.rid) //the dynamic set of regions of connection and the connection's current epochs
...
assume val frame_c_inv: c:connection -> h0:hh -> h1:hh -> Lemma 
  (requires (TLSMem.equal_on (c_regionsT c h0) h0 h1
	     /\ c_log_inv h0
	     /\ c_inv c h0))
