...
val protect: (inputs:list (plain i ad rg)) ->
  ST (list block)
     (requires (fun h ->
         fp_inv h
      /\ writer_in_fp wr h
//      /\ (exists fpe. List.mem fpe (TLSMem.sel h fp) /\ FPEntry?.i fpe = i /\ FPEntry?.w fpe = wr)
      /\ (forall (n:nat) . (n <= Seq.length (sel h (StWriter.log wr)) + List.length inputs ==> is_seqn n))
      ))
     (ensures (fun h0 (cs:list block) h1 ->
        fp_inv h1
      /\ writer_in_fp wr h1
//      /\ (exists fpe. List.mem fpe (TLSMem.sel h1 fp) /\ FPEntry?.i fpe = i /\ FPEntry?.w fpe = wr)
