...
private let fake_aead pv aeAlg key iv plain =
  ...
  let w: writer id =
    assume (~(authId id));
    let seqn: TLSMem.ref seqn_t = ralloc r 1 in
    let st: AEAD_GCM.state id Writer =
      ...
      (* let log: TLSMem.rref _ = ralloc r Seq.createEmpty in *)
...
private let fake_cbc pv aeAlg seqn key iv plain macKey =
  ...
  let w: ENC.encryptor id =
    ...
    let log: TLSMem.ref _ = ralloc r Seq.createEmpty in
    let state: ENC.localState r id =
      let iv: ENC.iv id = bytes_of_hex iv in
      ENC.OldBlockState id iv
    in
    let state: TLSMem.ref (ENC.localState r id) = ralloc r state in
