...
val epochs : c:connection -> h:TLSMem.mem -> GTot (es:seq (epoch (region_of c.hs) (random_of c.hs)){
...
//16-05-30 unused?
val frame_epochs: c:connection -> h0:TLSMem.mem -> h1:TLSMem.mem -> Lemma
  (requires (
    Map.contains (TLSMem.HS?.h h0) (region_of c.hs) /\ 
    equal_on (Set.singleton (region_of c.hs)) (TLSMem.HS?.h h0) (TLSMem.HS?.h h1)))
...
val equal_on_disjoint: 
  ...
  r:rid{Set.mem r s1} -> 
  h0:TLSMem.mem -> h1:TLSMem.mem{modifies (Set.singleton r) h0 h1} -> Lemma (equal_on s2 (TLSMem.HS?.h h0) (TLSMem.HS?.h h1))
