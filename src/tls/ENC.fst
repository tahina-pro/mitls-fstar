...
noeq type state (i:id) (rw:rw) = | StateB:
  #region: rid ->
  #peer_region: rid { TLSMem.disjoint region peer_region } -> 
...
val gen: 
  ...
  (requires (fun h0 -> TLSMem.disjoint reader_parent writer_parent))
...
let gen reader_parent writer_parent i =
  ...
  assert(TLSMem.disjoint reader_r writer_r);
...
//   b2t (List.mem (Entry i ad c p) (TLSMem.sel h log))
