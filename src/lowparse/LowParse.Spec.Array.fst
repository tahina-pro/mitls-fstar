...
let fldata_to_array_correct
  ...
  (u2: unit { parse_fldata_strong_pred (serialize_list s ()) array_byte_size x } )
: Lemma
  (ensures (
    array_pred elem_count x
  ))
= let y = serialize (serialize_list s ()) x in
  assert (parse (parse_list p ()) y == Correct (x, array_byte_size));
...
let fldata_to_array
  ...
  (x: parse_fldata_strong_t (serialize_list s ()) array_byte_size)
...
: Lemma
  (forall (x1 x2: parse_fldata_strong_t (serialize_list s ()) array_byte_size) .
...
= let (u : unit { fldata_array_precond p array_byte_size elem_count == true } ) = () in
  ...
  parse_fldata_strong (serialize_list s ()) array_byte_size e `parse_synth` (fldata_to_array s array_byte_size elem_count u)
...
: Lemma
  (ensures (
    parse_fldata_strong_pred (serialize_list s ()) array_byte_size x
  ))
= let y = serialize (serialize_list s ()) x in
  list_length_constant_size_parser_correct p () y
...
: Tot (parse_fldata_strong_t (serialize_list s ()) array_byte_size)
= [@inline_let]
  let (x' : list t) = x in
  [@inline_let]
...
= fldata_to_array_inj s array_byte_size elem_count u;
  ...
  serialize_synth
    ...
    (serialize_fldata_strong (serialize_list s ()) array_byte_size e)
...
let vldata_to_vlarray_correct
  ...
  (u2: unit { parse_bounded_vldata_strong_pred array_byte_size_min array_byte_size_max (serialize_list s ()) x } )
: Lemma
  (ensures (
...
let vldata_to_vlarray
  ...
  (x: parse_bounded_vldata_strong_t array_byte_size_min array_byte_size_max (serialize_list s ()))
...
: Lemma
  (forall (x1 x2: parse_bounded_vldata_strong_t array_byte_size_min array_byte_size_max (serialize_list s ())) .
...
= vldata_to_vlarray_inj array_byte_size_min array_byte_size_max s elem_count_min elem_count_max u;
  parse_bounded_vldata_strong array_byte_size_min array_byte_size_max (serialize_list s ())
...
: Lemma
  (ensures (
    parse_bounded_vldata_strong_pred array_byte_size_min array_byte_size_max (serialize_list s ()) x
  ))
= let y = serialize (serialize_list s ()) x in
  let l = L.length x in
  assert (parse (parse_list p ()) y == Correct (x, Seq.length y));
...
: Tot (parse_bounded_vldata_strong_t array_byte_size_min array_byte_size_max (serialize_list s ()))
= [@inline_let]
  let x' : list t = x in
  [@inline_let]
...
: Tot (serializer (parse_vlarray array_byte_size_min array_byte_size_max s elem_count_min elem_count_max e_size_incomplete e_size_invalid e_payload u))
= vldata_to_vlarray_inj array_byte_size_min array_byte_size_max s elem_count_min elem_count_max u;
  vlarray_to_vldata_to_vlarray array_byte_size_min array_byte_size_max s elem_count_min elem_count_max u;
  serialize_synth
    _
    (vldata_to_vlarray array_byte_size_min array_byte_size_max s elem_count_min elem_count_max u)
    (serialize_bounded_vldata_strong array_byte_size_min array_byte_size_max (serialize_list s ()) e_size_incomplete e_size_invalid e_payload)
